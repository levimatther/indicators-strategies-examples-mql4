   ...
   if(IsNewBar && IsSpreadGood && Count(0)+Count(1)<Max_Orders && signal==false)
     {
      ...
      if(Close[1]>MAvg(1) && RSI0 > 30 && RSI1 < 30) {OpenOrder(0); candletime=Time[0];}
      if(Close[1]<MAvg(1) && RSI0 < 70 && RSI1 > 70) {OpenOrder(1); candletime=Time[0];}
     }
...
double highest_lot = GetHighestOrderLotFromArray(index);
  if(highest_lot >0)
    ...
    if(Close[1]>MAvg(1) && RSI0 > 30 && RSI1 < 30) OpenOrder(0,NormalizeDouble(highest_lot*Multiplier,2));
    if(Close[1]<MAvg(1) && RSI0 < 70 && RSI1 > 70) OpenOrder(1,NormalizeDouble(highest_lot*Multiplier,2));
