...
input int    StopLoss=500; //Stoploss In Pips
...
double highest_lot = GetHighestOrderLotFromArray(index);
   ...
   double ma=iMA(_Symbol,0,MaPeriod,MaShift,Ma_Method,Apply_Price,0);
  ...
  if(highest_lot > 0)
    {
    if(Open[1]<ma && Close[1]>ma && RSI0 > 30 && RSI1 < 30) OpenOrder(0,NormalizeDouble(highest_lot*Multiplier,2));
    if(Open[1]>ma && Close[1]<ma && RSI0 < 70 && RSI1 > 70) OpenOrder(1,NormalizeDouble(highest_lot*Multiplier,2));
